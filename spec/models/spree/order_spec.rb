...
describe Spree::Order do
  ...
  let(:order) {
    create(:order_with_line_items, state: 'payment', line_items_count: 6, line_items_price: 100, shipment_cost: 50)
  }
  ...
  context 'vendor methods' do
    let!(:shipment) {
      create(:shipment, order: order, cost: 20, pre_tax_amount: 20, stock_location: vendor.stock_locations.first)
    }
