...
describe Spree::VendorMailer, type: :mailer do
  ...
  let(:vendor) {
    Spree::Vendor.new(id: 1, name: 'Test vendor', state: 'active', notification_email: 'test-vendor@example.com')
  }
  let(:other_vendor) {
    Spree::Vendor.new(id: 2, name: 'Other vendor', state: 'active', notification_email: 'other-vendor@example.com')
  }
  ...
  context 'vendor notification email' do
    ...
    it 'is sent from current store email address' do
      notification_email = described_class.vendor_notification_email(order.id, vendor.id)
    ...
    it 'body contains vendors items' do
      notification_email = described_class.vendor_notification_email(order.id, vendor.id)
      vendors_item = order.line_items.find { |line_item| line_item.product.vendor == vendor }
    ...
    it 'body does not contain other vendors items' do
      notification_email = described_class.vendor_notification_email(order.id, vendor.id)
      other_vendors_item = order.line_items.reject { |line_item| line_item.product.vendor == vendor }.first
